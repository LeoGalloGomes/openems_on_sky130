...
# Modification of layer recognition function to allow usage on sky130
# PDK layers on 05/01/2024 Leonardo Gomes
...
if len(sys.argv) >= 2:
  ...
  # iterate over sky130 technology layers
  for layer_to_extract in layerlist:
		
	  print ("Evaluating layer ", str(layer_to_extract))
		
	  for purpose_to_extract in purposelist:
		
		  print ("Evaluating purpose ", str(purpose_to_extract))
		  ...
		  if (layer_to_extract in used_layers):
		    ...
		    for LPP in LPPpolylist:
		      ...
		      if (layer==layer_to_extract) and (purpose==purpose_to_extract):
		        layername = layernum2layername(layer, purpose)
