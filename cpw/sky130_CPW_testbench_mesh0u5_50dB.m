...
% Cell ("test_CPW", 3 polygons, 0 paths, 0 labels, 0 references)
...
clear p;
p(1,1) = cpw_gap+cpw_sig/2;
p(2,1) = 0.000;
p(1,2) = cpw_gap+cpw_sig/2;
p(2,2) = 500.000;
p(1,3) = cpw_gap+cpw_sig/2+cpw_gnd;
p(2,3) = 500.000;
p(1,4) = cpw_gap+cpw_sig/2+cpw_gnd;
p(2,4) = 0.000;
CSX = AddLinPoly( CSX, 'Metal5', 200, 'z', Metal5.zmin, p, Metal5.thick); 
clear p;
p(1,1) = -(cpw_gap+cpw_sig/2+cpw_gnd);
p(2,1) = 0.000;
p(1,2) = -(cpw_gap+cpw_sig/2+cpw_gnd);
p(2,2) = 500.000;
p(1,3) = -(cpw_gap+cpw_sig/2);
p(2,3) = 500.000;
p(1,4) = -(cpw_gap+cpw_sig/2);
p(2,4) = 0.000;
CSX = AddLinPoly( CSX, 'Metal5', 200, 'z', Metal5.zmin, p, Metal5.thick); 

% Bounding box of geometry
geometry.xmin= -(cpw_gap+cpw_sig/2+cpw_gnd);
geometry.xmax= cpw_gap+cpw_sig/2+cpw_gnd;
geometry.ymin= 0.000;
...
[CSX, port{1}] = AddLumpedPort( CSX, 500, 1, 100, [-cpw_sig/2 geometry.ymin Metal5.zmin], [-(cpw_sig/2+cpw_gap) (geometry.ymin) Metal5.zmax], [1 0 0], true);
[CSX, port{2}] = AddLumpedPort( CSX, 500, 2, 100, [cpw_sig/2 geometry.ymin Metal5.zmin], [(cpw_sig/2+cpw_gap) (geometry.ymin) Metal5.zmax], [1 0 0], false);

[CSX, port{3}] = AddLumpedPort( CSX, 500, 3, 100, [-cpw_sig/2 geometry.ymax Metal5.zmin], [-(cpw_sig/2+cpw_gap) (geometry.ymax) Metal5.zmax], [1 0 0], false);
[CSX, port{4}] = AddLumpedPort( CSX, 500, 4, 100, [cpw_sig/2 geometry.ymax Metal5.zmin], [(cpw_sig/2+cpw_gap) (geometry.ymax) Metal5.zmax], [1 0 0], false);
...
mesh.x = [linspace(-cpw_sig/2, cpw_sig/2, ceil(cpw_sig/refined_cellsize))]; % signal mesh
mesh.x = [mesh.x linspace(-cpw_sig/2, -cpw_sig/2-cpw_gap, 12) linspace(cpw_sig/2, cpw_sig/2+cpw_gap, 12)]; % gap mesh
mesh.x = [mesh.x linspace(-cpw_sig/2-cpw_gap, -cpw_sig/2-cpw_gap-cpw_gnd, 12) linspace(cpw_sig/2+cpw_gap, cpw_sig/2+cpw_gap+cpw_gnd, 12)]; % ground mesh
...
port = calcPort( port, Sim_Path, f, 'RefImpedance', 100);

s11 = (port{1}.uf.ref + port{2}.uf.ref) ./ (port{1}.uf.inc + port{2}.uf.inc);
s21 = (port{3}.uf.ref + port{4}.uf.ref) ./ (port{1}.uf.inc + port{2}.uf.inc);
...
% alpha
figure 3;
plot(f/1e9,alpha*8.68/1000,'r-','LineWidth',2);
grid on;
...
% Zc
figure 4;
plot(f/1e9,real(Zc),'r-','LineWidth',2);
grid on;
